...
<div class="container clearfix">
	...
	<div class="clearfix uc-content">
			...
			<?php }?>
			<div >
				<form class="box" action="<?php echo urldecode(Url::urlFormat("/ucenter/voucher_activated"));?>" method="post">
					<h2>实体优惠券激活:</h2>
				...
				<div class="tab-body">
					<div id="voucher-n" class="js-template">
						...
						<tbody class="page-content">
							<tr class="{odd-even}">
								<td>{account}</td> <td>{name}</td> <td><?php echo isset($currency_symbol)?$currency_symbol:"";?>{value}</td> <td><?php echo isset($currency_symbol)?$currency_symbol:"";?>{money}</td> <td>{start_time} ~ {end_time}</td>
							</tr>
						</tbody>
						</table>
...
<script type="text/javascript">
	$("#voucher-n").Paging({
                url:'<?php echo urldecode(Url::urlFormat("/ucenter/get_voucher"));?>',
                params:{status:'n'}
    });
    $("#voucher-u").Paging({
                url:'<?php echo urldecode(Url::urlFormat("/ucenter/get_voucher"));?>',
