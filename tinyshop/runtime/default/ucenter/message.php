...
<script type="text/javascript">
 function message_n(){
  $("#message-n").Paging({
    url:'<?php echo urldecode(Url::urlFormat("/ucenter/get_message"));?>',
    params:{status:'n'}
});
}
function message_y(){
...
function message_read(id){
 $.post("<?php echo urldecode(Url::urlFormat("/ucenter/message_read"));?>",{id:id},function(data){
...
function message_del(id){
 $.post("<?php echo urldecode(Url::urlFormat("/ucenter/message_del"));?>",{id:id},function(data){
  if(data['status']=='success'){
    message_y();
}
},'json')
