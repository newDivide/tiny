...
<div class="bg-base">
    <ol class="bread-crumb container">
      <li><a  href="<?php echo urldecode(Url::urlFormat("/index/index"));?>">首页</a></li>
      <li><a  href="<?php echo urldecode(Url::urlFormat("/index/help_index"));?>">帮助中心 </a></li>
    </ol>
</div>
<!-- E 面包屑导航 -->
<div class="container">
<div class="mt10 clearfix" style="position: relative;">
    ...
    <h2 class="header">帮助中心</h2>
        <div class="box mt20">
            <?php $help_category=array();?>
            <?php $item=null; $query = new Query("help");$items = $query->find(); foreach($items as $key => $item){?>
            <?php $help_category[$item['category_id']][]=$item;?>
            <?php }?>
            <?php $item=null; $query = new Query("help_category");$items = $query->find(); foreach($items as $key => $item){?>
            <h2><?php echo isset($item['name'])?$item['name']:"";?></h2>
            <ul class="menu-list">
                <?php if(isset($help_category[$item['id']])){?>
                <?php foreach($help_category[$item['id']] as $key => $item){?>
                <li><a <?php if($id==$item['id']){?>class="current"<?php }?>  href="<?php echo urldecode(Url::urlFormat("/index/help/id/$item[id]"));?>"><?php echo isset($item['title'])?$item['title']:"";?></a></li>
                <?php }?>
                <?php }?>
            </ul>
            <?php }?>
	...
	<?php $query = new Query("help");$query->where = "id = $id";$items = $query->find();?>
		<?php if(count($items)>0){?>
		<div class="help-content p20">
            <?php foreach($items as $key => $item){?>
			<h1 class="title"><span class="f16"><?php echo isset($item['title'])?$item['title']:"";?></span></h1>
			<div class="mt10">
		...
		<div class="mt10">
            ...
            <dd class="help-list">
                ...
                <?php if(isset($help_category[$item['id']])){?>
                <?php foreach($help_category[$item['id']] as $key => $item){?>
                <li><a <?php if($id==$item['id']){?>class="current"<?php }?>  href="<?php echo urldecode(Url::urlFormat("/index/help/id/$item[id]"));?>"><?php echo isset($item['title'])?$item['title']:"";?><span class="l-triangle"></span></a></li>
                <?php }?>
                <?php }?>
                </ul>
            </dd>
