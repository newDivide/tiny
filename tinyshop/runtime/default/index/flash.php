...
<body>
    ...
    <div class="container">
        ...
        <ol class="bread-crumb">
            <li><a href="#">限时▪抢购</a></li>
            <li><a href="#">特卖▪专场</a></li>
        </ol>
        <ul class="product-list clearfix">
            <?php $item=null; $flashbuy = new Query("flash_sale as gb");$flashbuy->fields = "*,gb.id as id";$flashbuy->join = "left join goods as go on gb.goods_id = go.id";$flashbuy->order = "is_end,goods_num desc";$flashbuy->limit = "12";$flashbuy->page = "1";$items = $flashbuy->find(); foreach($items as $key => $item){?>
            <li class="item">
                <dl class="product">
                    <dt class="img"><a href="<?php echo urldecode(Url::urlFormat("/index/flashbuy/id/$item[id]"));?>"><img src="<?php echo Common::thumb($item['img'],600,360);?>" ></a></dt>
                    <dd class="title"><a href="<?php echo urldecode(Url::urlFormat("/index/flashbuy/id/$item[id]"));?>"><?php echo TString::msubstr($item['title'],0,15);?></a></dd>
                    <dd class="price"><i><?php echo isset($currency_symbol)?$currency_symbol:"";?></i><?php echo isset($item['price'])?$item['price']:"";?></dd>
                    <dd class="status"><?php if($item['is_end']==0){?><a class="btn btn-main" href="<?php echo urldecode(Url::urlFormat("/index/flashbuy/id/$item[id]"));?>">立即抢购</a><?php }else{?><span class="btn disabled btn-gray">抢购结束</span><?php }?></dd>
                </dl>
            </li>
            <?php }?>
        </ul>
        <div class=" mt10 page-nav"><?php echo $flashbuy->pageBar();?></div>
        <!--E 产品展示-->
    </div>
</div>
