...
<?php $parse = array('weixin'=>'微信','email'=>'邮件','sms'=>'短信');?>
...
<table class="default" >
    ...
    <?php foreach($notices as $key => $item){?>
        <tr><td ><?php echo isset($item[0]['title'])?$item[0]['title']:"";?></td><td class="btn_min">
        <?php foreach($item as $key => $obj){?>
        <a class="icon-pencil <?php if($obj['status']==1){?>green<?php }else{?>red<?php }?>" href="<?php echo urldecode(Url::urlFormat("/admin/notice_template_edit/id/$obj[id]"));?>"> <?php echo isset($parse[$obj['style']])?$parse[$obj['style']]:"";?></a><?php }?></td></tr>
    <?php }?>
</table>
</form>
