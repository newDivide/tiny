...
<div class="tab-body">
    <div>
        ...
        <table class="default" style="border-top: 0">
            <?php $item=null; $query = new Query("oauth");$query->order = "sort desc";$items = $query->find(); foreach($items as $key => $item){?>
                <tr>
                  <td class="btn_min"><?php echo isset($item['name'])?$item['name']:"";?></td>
                  <td class="btn_min"><img src="<?php echo urldecode(Url::urlFormat("@protected/classes/oauth/logo/$item[icon]"));?>"></td>
                  <td class="btn_min"><?php if($item['status']==1){?><b class="green">已开启</b><?php }else{?><b class="gray">未开启</b><?php }?></td>
                  <td class="btn_min"><?php echo isset($item['sort'])?$item['sort']:"";?></td>
                  <td style="width:80px;"><a href="<?php echo urldecode(Url::urlFormat("/admin/oauth_edit/id/$item[id]"));?>">编辑</a></td></tr>
            <?php }?>
        </table>
        </div>
        </form>
