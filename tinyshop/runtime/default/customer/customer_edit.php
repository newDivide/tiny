...
<div id="obj_form" class="form2">
    <form action="<?php echo urldecode(Url::urlFormat("/customer/customer_save"));?>" method="post" >
    ...
    <dl class="lineD">
      ...
      <dd>
        ...
        <?php }else{?>
        <input name="name" type="text" pattern="required" value="<?php echo isset($name)?$name:"";?>" alt="用户名称（必填）" />
        <label></label>
        <?php }?>
      </dd>
      </dl>
      ...
      <dd>
        <?php if( isset($id) &&  isset($email)){?>
        <label><?php echo isset($email)?$email:"";?></label>
        <?php }else{?>
        <input name="email" type="text"  pattern="email" value="<?php echo isset($email)?$email:"";?>" alt="邮箱地址(例如:tiny@tiny.com)">
        <label></label>
        <?php }?>
      </dd>
      </dl>
      ...
      <dt>性别：</dt>
      <dd>
        <input name="sex" type="radio" value="0" checked="checked"> <label> 女</label>&nbsp;&nbsp;<input name="sex" type="radio" <?php if(isset($sex) && $sex==1){?>checked="checked"<?php }?> value="1"> <label> 男</label>
      </dd>
      </dl><dl class="lineD">
      <dt>出生日期：</dt>
      <dd>
        <input name="birthday" type="text" onfocus="WdatePicker()" class="Wdate"  value="<?php echo isset($birthday)?$birthday:"";?>" >
        <label></label>
      </dd>
      </dl><dl class="lineD">
      <dt>地区：</dt>
      ...
      <dt>固话：</dt>
      <dd>
        <input name="phone" type="text" empty pattern="phone" value="<?php echo isset($phone)?$phone:"";?>">
        <label></label>
      </dd>
      </dl><dl class="lineD">
      <dt>手机：</dt>
...
function changePassword(e){
  ...
  var id = $("#objId").val();
  if(!e){
    $.post("<?php echo urldecode(Url::urlFormat("/customer/customer_password"));?>",{id:id,password:password,repassword:repassword},function(data){
