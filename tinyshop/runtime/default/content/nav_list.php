...
<div class="tools_bar clearfix">
    ...
    <a  class="icon-remove-2" href="javascript:;" onclick="tools_submit({action:'<?php echo urldecode(Url::urlFormat("/content/nav_del"));?>',msg:'删除后无法恢复，你真的删除吗？'})" title="删除"> 删除</a>
    <a href='<?php echo urldecode(Url::urlFormat("/content/nav_edit"));?>' class="icon-plus" > 添加</a>
</div>
<table class="default">
    <tr>
        ...
        <th style="width:160px">是否开启</th>
        
    </tr>
    <?php $item=null; $obj = new Query("nav");$obj->page = "1";$obj->order = "type desc,`sort` desc";$items = $obj->find(); foreach($items as $key => $item){?>
        <tr><td style="width:30px"><input type="checkbox" name="id[]" value="<?php echo isset($item['id'])?$item['id']:"";?>"></td>
        <td style="width:70px" class="btn_min"><div class="operat hidden"><a  class="icon-cog action" href="javascript:;"> 处理</a><div class="menu_select"><ul>
                    <li><a class="icon-pencil" href="<?php echo urldecode(Url::urlFormat("/content/nav_edit/id/$item[id]"));?>"> 编辑</a></li>
                    <li><a class="icon-remove-2" href="javascript:confirm_action('<?php echo urldecode(Url::urlFormat("/content/nav_del/id/$item[id]"));?>')"> 删除</a></li>
                </ul></div></div> </td>
        <td><?php echo isset($item['name'])?$item['name']:"";?></td><td><span><?php if($item['type']=='main'){?>主导航<?php }else{?>底部导航<?php }?></span></td><td><span><?php if($item['open_type']==1){?>新窗口<?php }else{?>本窗口<?php }?></span></td><td><?php echo isset($item['sort'])?$item['sort']:"";?></td><td><?php if($item['enable']==1){?><b class="green">开启</b><?php }else{?><span>关闭</span><?php }?></td></tr>
    <?php }?>
</table>
<div class="page_nav">
